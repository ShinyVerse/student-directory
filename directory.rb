...
def save_to_gitignore
  file = File.open(file_path('.gitignore'), "w")
  file.puts "directories.csv\n#{@known_directories.join("\n")}"
  file.close
end
...
def save_directories
  file = File.open(file_path('directories.csv'), "w")
...
def try_load_directories
  filename = ARGV.first || file_path("directories.csv")
